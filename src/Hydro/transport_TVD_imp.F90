...
!     Working temporary arrays in this routine
     ...
     &swild(max(3,nvrt)),swild4(3,2),trel_tmp_outside(ntr)
      ...
      real(rkind) :: vnor1,vnor2,xcon,ycon,zcon,dot1,sum1,tmp,cwtmp,toth, &
                     ...
                     &av_h,difnum,cwtmp2,dt_by_bigv,dtb_by_bigv
...
!...  Fei: below is an e.g. of implicit upwind -feel free to add arrays
!     if needed
      do i=1,ne
        if(idry_e(i)==1) cycle
...
!@         endif
            ...
            !Upwind advection
            if(flux_adv_vface(k,i)<0) then !inflow
              tmp=abs(flux_adv_vface(k,i))*dt_by_bigv !flux_mod(:) all same for upwind
              cupp(kin)=cupp(kin)-tmp
              bdia(kin)=bdia(kin)+tmp
            endif !flux_adv_vface
          endif !k<nvrt
...
!         Calculate RHS
          rrhs(1:ntr,kin)=trel_tmp(1:ntr,k,i)

!         Body source
          rrhs(1:ntr,kin)=rrhs(1:ntr,kin)+dt*bdy_frc(1:ntr,k,i)
...
!         b.c.
          if(k==nvrt) rrhs(1:ntr,kin)=rrhs(1:ntr,kin)+area(i)*dt_by_bigv*flx_sf(1:ntr,i)
          if(k==kbe(i)+1) rrhs(1:ntr,kin)=rrhs(1:ntr,kin)-area(i)*dt_by_bigv*flx_bt(1:ntr,i)
        enddo !k=kbe(i)+1,nvrt
