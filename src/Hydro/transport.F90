...
!                            &ifltype(max(1,nope_global)),itetype(max(1,nope_global)), &
!                            &isatype(max(1,nope_global)),itrtype(max(1,nope_global)), &
      real(rkind), intent(in) :: dfh1(nvrt1,npa1) !indices reversed from dfh()
!                                 tobc(nope_global),sobc(nope_global),trobc(nope_global), &
...
!     Working temporary arrays in this routine
      ...
      real(rkind) :: sne(nvrt,3),area_e(nvrt),psumtr(ntr),delta_tr(ntr),adv_tr(ntr), &
        ...
        do k=kbe(i)+1,nvrt
          ...
          if(k<nvrt) then
            ...
            if(imod==1) then
              ...
              cupp(kin)=cupp(kin)-area(i)*sum(wsink(1,k,nm(i,1:3)))/3*dtb/bigv !2nd number <0
            endif
#endif /*USE_TIMOR*/
          endif !k<nvrt
          ...
          if(k>kbe(i)+1) then
            ...
            if(imod==1) then
              !Sink vel.
              !Error: need to differentiate tracer index
              bdia(kin)=bdia(kin)+area(i)*sum(wsink(1,k-1,nm(i,1:3)))/3*dtb/bigv !2nd number >0
            endif
#endif /*USE_TIMOR*/
          endif !k>kbe(i)+1
...
!         Alternative mass conservative form for the advection part (Eq. C32); contribute to rrhs
          ...
          if(ntr>1) then; if(flx_mod(1,k,2,i)*flx_mod(2,k,2,i)<0) then
...
!         Horizontal faces
     ...
     &(1-trobc(ibnd))*trel_tmp(:,k,i)
            ...
            if(ntr>1) then; if(flx_mod(1,k,1,jsj)*flx_mod(2,k,1,jsj)<0) then
