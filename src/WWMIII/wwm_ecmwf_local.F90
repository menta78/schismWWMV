...
!     ------------------------------------------------------------
     ...
     &                      RHOAIR => RHOA
      ...
      IF(IPHYS.EQ.0) THEN
        IF (MESIN .GT. 0) CALL SINPUT_LOCAL (IPP, FL3, FL, THWNEW(IPP), USNEW(IPP), Z0NEW(IPP), ROAIRN(IPP), ZIDLNEW(IPP), SL, XLLWS, SSIN, DSSIN)
      ELSE
        IF (MESIN .GT. 0) CALL SINPUT_ARD_LOCAL (IPP, FL3, FL, THWNEW(IPP), USNEW(IPP), Z0NEW(IPP), ROAIRN(IPP), ZIDLNEW(IPP), SL, XLLWS, SSIN, DSSIN)
      ENDIF
      ...
      ELSE
        IF (MESDS .GT. 0) CALL SDISS_ARDH_VEC_LOCAL (IPP, FL3 ,FL, SL, F1MEAN, XKMEAN,PHIOC, TAUWD, MIJ, SSDS, DSSDS)
      ENDIF
      DO K = 1, NANG
        DO M = 1, NFRE
          SSDSO(M,K)   = SSDS(K,M)
