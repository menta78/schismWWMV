...
!**********************************************************************
      ...
      CALL COMPUTE_PHI_DPHI(IP,WALOC,SSINL,SSINE,DSSINE,SSDS,DSSDS,SSNL3,DSSNL3,SSNL4,DSSNL4,SSBR,DSSBR,SSBRL,SSBF,DSSBF,PHI,DPHIDN)
      ...
      CALL POST_INTEGRATION(IP,WACNEW,SSINL,SSINE,DSSINE,SSDS,DSSDS,SSNL3,DSSNL3,SSNL4,DSSNL4,SSBR,DSSBR,SSBRL,SSBF,DSSBF,PHI,DPHIDN)

      END SUBROUTINE
!**********************************************************************
!*                                                                    *
!**********************************************************************
      SUBROUTINE COMPUTE_PHI_DPHI(IP,WALOC,SSINL,SSINE,DSSINE,SSDS,DSSDS,SSNL3,DSSNL3,SSNL4,DSSNL4,SSBR,DSSBR,SSBRL,SSBF,DSSBF,PHI,DPHIDN)
      USE DATAPOOL
      IMPLICIT NONE
      INTEGER, INTENT(IN)      :: IP
      INTEGER                  :: IS, ID
...
!*                                                                    *
!**********************************************************************
      SUBROUTINE POST_INTEGRATION(IP,WALOC,SSINL,SSINE,DSSINE,SSDS,DSSDS,SSNL3,DSSNL3,SSNL4,DSSNL4,SSBR,DSSBR,SSBRL,SSBF,DSSBF,PHI,DPHIDN)
         USE DATAPOOL
         IMPLICIT NONE
         ...
         REAL(rkind), INTENT(IN)   :: SSINL(NUMSIG,NUMDIR)
         REAL(rkind), INTENT(IN)   :: SSINE(NUMSIG,NUMDIR),DSSINE(NUMSIG,NUMDIR)
         REAL(rkind), INTENT(IN)   :: SSDS(NUMSIG,NUMDIR),DSSDS(NUMSIG,NUMDIR)
         REAL(rkind), INTENT(IN)   :: SSNL3(NUMSIG,NUMDIR),DSSNL3(NUMSIG,NUMDIR)
         REAL(rkind), INTENT(IN)   :: SSNL4(NUMSIG,NUMDIR),DSSNL4(NUMSIG,NUMDIR)
         REAL(rkind), INTENT(IN)   :: SSBR(NUMSIG,NUMDIR),DSSBR(NUMSIG,NUMDIR)
         REAL(rkind), INTENT(IN)   :: SSBRL(NUMSIG,NUMDIR)
         REAL(rkind), INTENT(IN)   :: SSBF(NUMSIG,NUMDIR),DSSBF(NUMSIG,NUMDIR)

         IF (ISOURCE == 1) THEN
           CALL ST4_POST(IP,WALOC,SSINL,SSINE,DSSINE,SSDS,DSSDS,SSNL3,DSSNL3,SSNL4,DSSNL4,SSBR,DSSBR,SSBRL,SSBF,DSSBF,PHI,DPHIDN)
...
!**********************************************************************
      SUBROUTINE SOURCES_EXPLICIT
         ...
         IMPLICIT NONE
         INTEGER :: IP
         REAL(rkind) :: WACOLDLOC(NUMSIG,NUMDIR), WACNEWLOC(NUMSIG,NUMDIR)
         ...
         DO IP = 1, MNP
           IF (IOBDP(IP) .GT. 0) THEN ! H .gt. DMIN
             ...
             IF (LSOUBOUND .AND. IOBP(IP) .NE. 2) THEN ! CALL ALWAYS 
               CALL SEMI_IMPLICIT_INTEGRATION(IP,DT4S,WACOLDLOC,WACNEWLOC)
             ...
             IF (MELIM .EQ. 1) THEN
               CALL GET_MAXDAC(IP,MAXDAC)
               CALL ACTION_LIMITER_LOCAL(MAXDAC,WACOLDLOC,WACNEWLOC,SSLIM)
             ENDIF
             IF (LMAXETOT) CALL BREAKING_LIMITER_LOCAL(IP,WACNEWLOC,SSBRL)
             AC2(:,:,IP) = WACNEWLOC
           ELSE
             AC2(:,:,IP) = ZERO
           ENDIF
      ...
      SUBROUTINE SOURCES_IMPLICIT
         ...
         DO IP = 1, MNP
           IF (IOBDP(IP) .GT. 0) THEN ! H .gt. DMIN
             ...
             WACOLD = AC2(:,:,IP)
             IF (LSOUBOUND  .AND. IOBP(IP) .NE. 2) THEN ! CALL ALWAYS
               CALL COMPUTE_PHI_DPHI(IP,WACOLD,SSINL,SSINE,DSSINE,SSDS,DSSDS,SSNL3,DSSNL3,SSNL4,DSSNL4,SSBR,DSSBR,SSBRL,SSBF,DSSBF,PHI,DPHIDN)
             ELSE IF (IOBP(IP) .EQ. 0 .AND. .NOT. LSOUBOUND) THEN ! CALL ONLY FOR NON BOUNDARY POINTS
               CALL COMPUTE_PHI_DPHI(IP,WACOLD,SSINL,SSINE,DSSINE,SSDS,DSSDS,SSNL3,DSSNL3,SSNL4,DSSNL4,SSBR,DSSBR,SSBRL,SSBF,DSSBF,PHI,DPHIDN)
             ENDIF
             IF (MELIM .EQ. 2) THEN
