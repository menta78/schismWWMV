...
!!            ----------------------------------------------------------
              ...
              tnh2 = MyTANH(wn2d)                   !* tanh(k2*depth)
...
!!
      ...
      f1    = sqrt(g*wk1*MyTANH(wk1*dep))/twopi  !* f1=f(k1,dep)
!!
      fx    = 4. * f1                          !* fx = 4*f1
...
!sq
      thp  = atan2(py,px)
      q    = sqrt(w3*MyTANH(w3)) - sqrt(w1*MyTANH(w1))
      qrtp = q / sqrt(p)
      qsqp = qrtp**2
!!
!!
...
!!
        ...
        dbt3    = (dbz**2) + 1.d0
        dbt4    = dbt3 / (2.d0*dbz)
        dbt5    = ((dsqrt(dbz*dMyTANH(dbz*dbp))-dbqrtp)**4)/(2.d0*dbz)
...
!!
        do 70 n=1,25
          cdthnew = dbt4 - dbt5 / ((MyTANH(dbt6))**2)
...
!!
        om1 = dsqrt(k1*MyTANH(k1))         !* norm. omega (by sqrt(h/g))
        om2 = dsqrt(k2*MyTANH(k2))
        om3 = dsqrt(k3*MyTANH(k3))
!!
        om1sq = om1**2
        om2sq = om2**2
        om3sq = om3**2
...
!!
        ...
        omsq23 = k23 * MyTANH(k23)         !* norm sq frq of v.k2+v.k3
