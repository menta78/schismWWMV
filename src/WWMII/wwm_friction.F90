...
!*                                                                    *
!**********************************************************************
      SUBROUTINE SDS_BOTF_NEW(IP,ACLOC,IMATRA,IMATDA)
         USE DATAPOOL
         IMPLICIT NONE
         ...
         !IF (SUM(ACLOC) .ne. SUM(ACLOC)) WRITE(*,*) SUM(ACLOC), IP
         ...
         DO IS = 1, MSC
           ...
           !WRITE(*,'(2I10,10F20.10)') IP, IS, KDEP, DEP(IP), CFBOT, SUM(SSBF(IS,:)), (SPSIG(IS) / SINH(MIN(20.0_rkind,KDEP)))**2 
           DO ID = 1, MDC
             IMATDA(IS,ID) = SSBF(IS,ID)
             IMATRA(IS,ID) = - SSBF(IS,ID) * ACLOC(IS,ID)
           END DO
         END DO
