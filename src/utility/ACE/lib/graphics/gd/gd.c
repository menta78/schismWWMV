...
int gdSetLineWidth(gdImagePtr im, int width);
static void gdImageBrushApply(gdImagePtr im, int x, int y);
static void gdImageTileApply(gdImagePtr im, int x, int y);

static int gdLineWidth = 1;

int gdSetLineWidth(gdImagePtr im, int width)
{
    int oldWidth = gdLineWidth;
    gdLineWidth = width % 8;
    if (gdLineWidth == 0) {
	gdLineWidth = 1;
    }
...
void gdImageSetHLine(gdImagePtr im, int y, int x1, int x2, int color)
{
    int i;
    for (i=x1;i<=x2;i++) {
...
{
	int i, j, dx, dy, incr1, incr2, d, x, y, xend, yend, xdirflag, ydirflag;
	dx = abs(x2-x1);
	dy = abs(y2-y1);
	if (dy <= dx) {
		d = 2*dy - dx;
		...
		if (gdLineWidth == 1) {
		    ...
		    for (i=0;i<gdLineWidth;i++) {
		    for (j=0;j<gdLineWidth;j++) {
		        gdImageSetPixel(im, x + i - gdLineWidth / 2, y + j  - gdLineWidth / 2, color);
		    }
		    }
		}
		if (((y2 - y1) * ydirflag) > 0) {
...
void gdImageFillArc(gdImagePtr im, int cx, int cy, int w, int h, int s, int e, int color)
{
	int i, nsamples;
	int lx = 0, ly = 0;
	int w2, h2;
	...
	while (e < s) {
        ...
        nsamples = e - s + 1;
	x = (int *) malloc(nsamples * sizeof(int));
	y = (int *) malloc(nsamples * sizeof(int));
	for (i=s; (i <= e); i++) {
		int xx, yy;
		xx = ((long)cost[i % 360] * (long)w2 / costScale) + cx; 
		yy = ((long)sint[i % 360] * (long)h2 / sintScale) + cy;
		x[i] = xx;
		y[i] = yy;
	}
	polyfill(im,x,y,nsamples,color);
	free(x);
	free(y);
}
...
static int edge_order();
static void edge ();
static void edge_point ();
...
/* sort the edge points by row and then by col */
    ...
    for (n = 1; n < np; n++)
    {
        ...
        gdImageSetHLine(im, P[n].y, P[n-1].x, P[n].x, color);
        n++;
    }
