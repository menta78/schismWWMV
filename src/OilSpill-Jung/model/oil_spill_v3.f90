!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!       Oil Spill Modelling Code for data format 5 (SZ) based on ptrack2.
!       Author: Dr. Yun C. Jung (ycjung@hhu.ac.kr) and Joseph Zhang
!										
...
!										
!	Inputs: 
!          a) hgrid.ll (if ics=2 in spill.in) or hgrid.gr3 (if ics=1 in
!          spill.in)
!          b) vgrid.in
!          c) spill.in (see format below; also sample 'spill.in.sample')
!          d) *hvel.64, *vert.63,*elev.61, *tdff.63, *wind.62
!          e) (for advanced users) some constants specified near the
!          beginning of the main routine (e.g. 'di')

!       Input spill.in (example script to generate this input can be
!       found in gen_data.f90):
...
!         (4) ics,slam0,sfea0 - ics=1 (map projection); =2 (lat/lon);
!         (slam0,sfea0) are lon/lat of the CPP projection center
!         (5) h0,rnday,dtm,nspool,ihfskip,ndeltp - min. depth, # of days,
!             time step used in the original run, nspool and ihfskip
!             used in the run, # of sub-division to be used in the tracking;
...
!                              if ihdf=1, the diffusivity is calculated
!                              from Smagorinsky scheme with the coefficient given by
!                              horcon (but hdc is used for i.c.)
...
!         idp(i),st_p(i),xpar(i),ypar(i),zpar0(i) -
!                                 particle id, start time (sec),starting
!                                 x,y, and z relative to the instant f.s. (<=0).
!										
!	Output: spill.out (vis drogue format), fort.16 (screen), fort.11 (fatal errors).		
!										
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! ...  code description
!      oil_spill_v3.f90 modified in Nov. 2014 from oil_spill_v2.f90 to
!      account for quads and other new devs.

!   pgf90 -O2 -mcmodel=medium -Mbounds -Bstatic -o oil_spill_v3 oil_spill_v3.f90 ../../Utility/UtilLib/compute_zcor.f90 ../../Utility/UtilLib/schism_geometry.f90 
...
!...  	Important variables
        integer,save :: np,ne,ns,nvrt,mnei,ibf,istiff,ivcor,kz,nsig
      	real(kind=dbl_kind),save :: h0,rho0,dt
        real,save :: h_c,theta_b,theta_f,h_s
...
!...  Read in particles
      open(95,file='spill.in',status='old')
      read(95,*)
!      read(95,'(a48)') data_format
      read(95,*) nscreen
      ibf=1 !no backward track
      !read(95,*) ibf
      !if(iabs(ibf)/=1) then
      !  write(*,*)'Wrong ibf',ibf
      !  stop
...
!                        -2 - stranded at wet/dry interface, after some time has elapsed 
!                             (only in this case is inbr (ngbring elem) defined)
!                        -1 - permanently stranded at land bnd (hit there and some time has elapsed)
!                        2 - hit the open bnd
...
!...  Read in header 
      ...
      iths=(ifile-1)*nrec+1 !step # for the 1st time step output in ifile
      ...
      open(63,file=ifile_char//'_wind.62',access='direct',recl=nbyte)
      open(64,file=ifile_char//'_tdff.63',access='direct',recl=nbyte)

      irec00=5*48/nbyte+5+7+nvrt+2 
!     Read initial bottom index
      do m=1,np
        ...
        !Debug
        !write(97,*)m,real(x(m)),real(dp(m)),kbp00(m)
      enddo !m=1,np
      do m=1,ne
        irec00=irec00+1
        do mm=1,i34(m)
...
!          endif
      ...
      open(95,file='spill.out',status='replace')
      write(95,*)'Drogues'
      write(95,*) nt-iths+1
...
!...  Read in elevation and vel. info
      ...
      irec1=irec1+np+2 !skip first part
      irec2=irec2+np+2
      irec3=irec3+np+2
      irec4=irec4+np+2
      irec5=irec5+np+2
...
!      call pt_in_poly3(i34(ntmp),x(elnode(1:i34(ntmp),ntmp)),y(elnode(1:i34(ntmp),ntmp)), &
!     &xpar(1),ypar(1),arco,nodel)
!      windx=0; windy=0; selev=0
!      do j=1,3
!        nd=elnode(nodel(j),ntmp)
...
!...  Store info for first step
      if(it==iths) then
        uu1=uu2; vv1=vv2; ww1=ww2; eta1=eta2; eta3=eta1
        vf1=vf2; wnx1=wnx2; wny1=wny2
      endif 

!...  Compute z-cor; after this kbp() is assigned  for ivcor=2 as well
!     (for ivcor=1, it has been assigned before)
      call levels
...
!            enddo !j                 
            dudx_e=dot_product(uu2(elnode(1:3,i),k),dldxy(1:3,1,i)) !1./(2.*area(i))*dux
            dudy_e=dot_product(uu2(elnode(1:3,i),k),dldxy(1:3,2,i)) !1./(2.*area(i))*duy
            dvdx_e=dot_product(vv2(elnode(1:3,i),k),dldxy(1:3,1,i)) !1./(2.*area(i))*dvx
            dvdy_e=dot_product(vv2(elnode(1:3,i),k),dldxy(1:3,2,i)) !1./(2.*area(i))*dvy
            hvis_e(i,k)=horcon*area(i)*sqrt(dudx_e**2+dvdy_e**2 & !m^2/s
                       &+.5*(dudy_e+dvdx_e)**2)
          enddo !k
        enddo !i=1,ne
...
!...  Particle tracking
      ...
      do i=1,nparticle
        ...
        if(time<=st_p(i)) go to 449 !output directly
...
!         Compute the vertical gradient of diffusivity
          ...
          if(tmp==0) then
            write(11,*)'Zero layer:',i,nnel,jlev
            stop
          endif
          grdz(i)=az/tmp
          !Gradient of viscosity
          grdx(i)=dot_product(van(1:3),dldxy(1:3,1,nnel))
          grdy(i)=dot_product(van(1:3),dldxy(1:3,2,nnel))
...
!
      ...
      integer :: idry_new(np) !,out2(12+mnv)
      real :: zlcl(nvrt)
