...
##########################################################################################
  FCPFLAGS = $(PPFLAGS) -O1 -finit-local-zero -ffree-line-length-none -fbacktrace -g -ggdb
  FCPFLAGS += -Wall -Wextra -Wconversion -Wno-unused  -Wno-unused-dummy-argument -Wno-unused-parameter
  FCPFLAGS += -fcheck=all
#  FCPFLAGS += -fsanitize=address -fsanitize=undefined 
  FCPFLAGS += -ffpe-trap=invalid,overflow
  FLDFLAGS = -O1
##########################################################################################
