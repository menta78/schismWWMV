...
FullNamelist NAMELIST_GetStandardMODEL_MERGING()
{
  ...
  BlockINPUT.BlockName=BlockName1;
  ListBlock["INPUT"]=BlockINPUT;
  // OUTPUT
  std::string BlockName2="OUTPUT";
  std::map<std::string, int> ListIntValues2;
  std::map<std::string, bool> ListBoolValues2;
  std::map<std::string, double> ListDoubleValues2;
  std::map<std::string, std::vector<double> > ListListDoubleValues2;
  std::map<std::string, std::string> ListStringValues2;
  std::map<std::string, std::vector<std::string> > ListListStringValues2;
  ListStringValues2["MODELNAME"]="COSMO or WAM or WWM";
  ListStringValues2["GridFile"]="COSMO_0001.nc";
  ListStringValues2["HisPrefix"]="not_set_";
  ListStringValues2["HisPrefixOut"]="FinalTarget_";
...
void INTERPOL_field_Function(FullNamelist const& eFull)
{
  ...
  std::vector<std::string> ListModelName=eBlINPUT.ListListStringValues.at("ListMODELNAME");
  std::vector<std::string> ListGridFile=eBlINPUT.ListListStringValues.at("ListGridFile");
  ...
  TripleModelDesc eTripleOut{eModelName, GridFile, HisPrefix, false, false, 0, 0};
  GridArray GrdArrOut=RETRIEVE_GRID_ARRAY(eTripleOut);
  TotalArrayInterpolation TotalArr=INTERPOL_ConstructTotalArray(ListTotalArr, ListSpongeSize, ListFatherGrid, GrdArrOut);
  std::string HisPrefixOut=eBlOUTPUT.ListStringValues.at("HisPrefixOut");
  // timings
  std::vector<double> ListTime=GetInterval(eBlOUTPUT.ListStringValues.at("BEGTC"),
                                           eBlOUTPUT.ListStringValues.at("ENDTC"),
                                           eBlOUTPUT.ListDoubleValues.at("DELTC"),
                                           eBlOUTPUT.ListStringValues.at("UNITC"));
  int nbTime=ListTime.size();
  ...
  //
  // Computing the dependency between the grids
  // 
  ...
  for (auto & eVarName : ListVarOut)
    if (eBlockVAR.ListBoolValues.at(eVarName))
      ListVarName.push_back(eVarName);
  for (int iTime=0; iTime<nbTime; iTime++) {
    double eTimeDay=ListTime[iTime];
    std::string eFileNC=HisPrefixOut + StringNumber(iFile,4) + ".nc";
    if (nbWritten == 0)
      INTERPOL_NetcdfInitialize(eFileNC, GrdArrOut, ListVarName);
    std::vector<RecVar> ListRecVar;
    for (auto & eVarName : ListVarName) {
      RecVar eRecVar=INTERPOL_MultipleRecVarInterpolation(TotalArr, eVarName, eTimeDay);
      ListRecVar.push_back(eRecVar);
    }
    INTERPOL_NetcdfAppendVarName(eFileNC, GrdArrOut, ListRecVar);
    nbWritten++;
    if (nbWritten == eMult) {
      iFile++;
      nbWritten=0;
...
int main(int argc, char *argv[])
{
  ...
  if (argc != 2) {
    fprintf(stderr, "INTERPOL_field is used as\n");
    fprintf(stderr, "INTERPOL_field [file.nml]\n");
    fprintf(stderr, "with file.nml the file describing the interpolation process\n");
    return -1;
  }
