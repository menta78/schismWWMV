...
FullNamelist NAMELIST_GetStandardALTIMETRY_COMPARISON()
{
  ...
  std::map<std::string, std::string> ListStringValues1;
  std::map<std::string, std::vector<std::string> > ListListStringValues1;
  ListStringValues1["MODELNAME"]="COSMO or WAM or ROMS or WW3";
...
void COMPUTE_STATUS_LON_LAT(std::vector<int> &ListStatus, MyVector<double> const& ListLon, MyVector<double> const& ListLat, FullNamelist const& eFull)
{
  ...
  for (int iMeas=0; iMeas<nbMeas; iMeas++)
	...
	//	std::cerr << "iMeas=" << iMeas << " IsInside=" << IsInside << " eLon=" << eLon << " eLat=" << eLat << "\n";
	eStatus=0;
	if (IsInside) 
	  eStatus=1;
	ListStatus[iMeas]=eStatus;
...
double GetGridSpacing(GridArray const& GrdArr)
{
  ...
  if (IsFE == 1) {
    std::cerr << "Missing code in GetGridSpacing\n";
    std::cerr << "Clearly, more programming is needed\n";
    exit(1);
  }
  else {
		...
		DrawLinesArr const& eDrawArr)
{
  ...
  OUTncl << "  ;  res@tiMainString     = \"Average Bathymetry modification\" \n";
  ...
  if (DoBackground) {
    OUTncl << "  lgres                    = True\n";
    OUTncl << "  lgres@lgLineColors     = (/\"foreground\", \"blue\"/)\n";
    OUTncl << "  lgres@lgItemType         = \"Lines\"\n";
    OUTncl << "  lgres@lgDashIndexes = (/0/)\n";
    OUTncl << "  lgres@lgLabelFontHeightF = .10\n";
    OUTncl << "  lgres@vpWidthF           = 0.45\n";
    OUTncl << "  lgres@vpHeightF          = 0.13\n";
...
std::vector<SingleRecInterp> FD_FIND_ELE(CoordGridArrayFD const& CoordGridArr, QuadCoordinate const& eQuad, MyMatrix<double> const& ListXY)
{
  ...
  std::function<SingleRecInterp(int, int, double, double)> FindRecord=[&](int const& eEta, int const& eXi, double const& eX, double const& eY) -> SingleRecInterp {
	...
	return {0, {}};
      ...
      for (std::vector<int> &ePair : ListCases) {
	int eEta=ePair[0];
	int eXi=ePair[1];
	...
	if (LCoeff[0] > -1) {
	  ...
	  for (int i=0; i<4; i++) {
	    ...
	    deltaX=deltaX - LCoeff[i]*CoordGridArr.LON(eEta, eXi);
	    deltaY=deltaY - LCoeff[i]*CoordGridArr.LAT(eEta, eXi);
	    LPart[i]=ePart;
	  }
	  //	  std::cerr << "deltaX=" << deltaX << " deltaY=" << deltaY << "\n";
	  return {1, LPart};
...
std::vector<SingleEntryAltimeter> READ_ALTI_FILE(std::string const& eFileAlti, std::vector<int> const& ListStatusSatellite, FullNamelist const& eFull)
{
  ...
  // time is special because of "days from ...." kind of things
...
void RAW_PLOT_ALTIMETER_TRACKS(std::ostream & os, std::vector<SatelliteListTrack> const& LTrack, PermanentInfoDrawing const& ePerm, FullNamelist const& eFull)
{
  ...
  MyMatrix<double> F(nbSplitLon, nbSplitLat);
  for (int iLon=0; iLon<nbSplitLon; iLon++)
    for (int iLat=0; iLat<nbSplitLat; iLat++)
      F(iLon, iLat)=double(iLon)/double(nbSplitLon);
  RecVar eRecVar;
